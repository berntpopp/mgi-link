...
Provenance is injected by the Docker image build (``MGI_LINK_GIT_SHA`` /
``MGI_LINK_BUILT_AT``). In a source checkout those env vars are absent, so the
...
def build_info() -> dict[str, str | None]:
    ...
    return {
        "version": __version__,
        "git_sha": os.environ.get("MGI_LINK_GIT_SHA") or _git_sha_from_dotgit() or "unknown",
        "built_at": os.environ.get("MGI_LINK_BUILT_AT") or _built_at_fallback(),
    }
