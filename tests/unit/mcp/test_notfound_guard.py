...
class _Stub:
    ...
    def close(self) -> None:  # set_mgi_service closes the previous service
        return None
...
def _service() -> Iterator[None]:
    set_mgi_service(_Stub())  # type: ignore[arg-type]
    try:
        yield
    finally:
...
async def _raw_request(method: str, params: dict[str, Any]) -> tuple[str, str]:
    """Drive one raw JSON-RPC request against the real mgi server (single consumer of
...
async def test_unknown_tool_name_not_reflected_via_client() -> None:
    ...
    # is_error must be flagged: an unknown tool has no output schema, so a non-error
    # structured result makes the client log the caller-supplied name.
...
async def test_unknown_resource_uri_via_client_severed_layer2() -> None:
    ...
    try:
        async with Client(mcp) as client:
            with pytest.raises(Exception) as excinfo:
                await client.read_resource("mgi://nonexistent-resource-xyz")
    finally:
        detach()
...
async def test_hostile_resource_uri_full_forbidden_set_no_leak(label: str, ch: str) -> None:
    ...
    response, logs = await _raw_request("resources/read", {"uri": f"mgi://{ch}evil-no-such"})
    _assert_no_leak(response, ch)
    _assert_no_leak(logs, ch)
