...
The entrypoint downloads the MGI reports and builds the local SQLite index
(~33 MB download → ~73 MB index, a few seconds) before the server starts. The
index is persisted in the `mgi-data` named volume across restarts.
...
```cron
17 3 * * *  docker compose -f /opt/mgi-link/docker/docker-compose.yml run --rm refresh
```
...
The host port defaults to `8000`; override with `MGI_LINK_HOST_PORT` (e.g. in
